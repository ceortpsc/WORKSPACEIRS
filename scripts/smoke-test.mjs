...
await check('certification overview','/certification',{},(response,body)=>response.status===200&&typeof body==='string'&&/Application PASS Evidence Center/i.test(body));
for(const page of ['application','efile','security','infrastructure','integrations'])await check(`certification ${page}`,`/certification/${page}`,{},response=>response.status===200);
await check('certification evidence API','/api/v1/certification/evidence',{},(response,body)=>response.status===200&&body?.ok===true&&body?.total>=20&&body?.counts&&Array.isArray(body?.evidence)&&body?.controls?.failClosed===true);
...
await check('anonymous masterfile refund sync denied','/api/v1/masterfiles/refund-sync',{method:'POST',headers:jsonHeaders,body:JSON.stringify({tenantId:'demo-tenant',cases:[]})},response=>[401,403,423].includes(response.status));
