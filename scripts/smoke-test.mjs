...
await check('anonymous registration denied','/api/v1/registrations',{method:'POST',headers:jsonHeaders,body:JSON.stringify({tenantId:'demo-tenant',registrationType:'TAXPAYER',legalName:'Demo User',email:'demo@example.com'})},response=>[401,403,423].includes(response.status));
await check('anonymous credential registry denied','/api/v1/admin/credentials',{method:'POST',headers:jsonHeaders,body:JSON.stringify({tenantId:'demo-tenant'})},response=>[401,403,423].includes(response.status));
await check('anonymous access decision denied','/api/v1/admin/access/authorize',{method:'POST',headers:jsonHeaders,body:JSON.stringify({tenantId:'demo-tenant'})},response=>[401,403,423].includes(response.status));
