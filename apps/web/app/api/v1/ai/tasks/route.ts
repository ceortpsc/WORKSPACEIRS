...
export async function POST(request:Request){
 ...
 try{
  const body=await readJson(request);const persona=String(body.persona??'');const instruction=String(body.instruction??'');const riskTier=String(body.riskTier??'low') as RiskTier;
  ...
  return apiResponse({ok:true,result,prohibitedActions:['sign','transmit','represent','change_bank_data','clear_material_hold','make_final_tax_or_legal_decision']},result.state==='HOLD'?409:202,context.correlationId);
