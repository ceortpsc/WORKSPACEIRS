...
export const metadata:Metadata={
 ...
 description:'Enterprise tax-practice operations, ERO workflows, ETRAC intelligence, payroll, compliance, and secure client services.',
 applicationName:'WORKSPACEIRS',
 robots:isIndexable?{index:true,follow:true}:{index:false,follow:false,noarchive:true},
 openGraph:{type:'website',siteName:'Ross Tax Pro Software Co.',title:'Smarter Software. Stronger Practice Results.',description:'A governed tax-practice operating system for taxpayers, preparers, EROs, and multi-office operations.',url:siteUrl},
...
export default function RootLayout({children}:{children:React.ReactNode}){
 return <html lang="en"><body>
  ...
  <header className="site-header"><div className="container nav">
   ...
   <nav className="nav-links" aria-label="Primary navigation">{primaryNavigation.map(route=><Link key={route.slug} href={`/${route.slug}`}>{route.title.replace('Tax and Software ','').replace('Professional ','')}</Link>)}<Link className="ross-btn ross-btn--gold nav-cta" href="/start">Start Secure Intake</Link></nav>
  </div></header>
  <main id="main-content">{children}</main>
  <footer className="footer"><div className="container footer-grid">
   <div><strong>Ross Tax Pro Software Co.</strong><p>Smarter Software. Stronger Practice Results.</p><p className="footer-note">Enterprise tax operations, secure client service, governed integrations, ETRAC intelligence, and practitioner software.</p></div>
  ...
  </div><div className="container footer-bottom"><span>© 2026 Ross Tax Pro Software Co. All rights reserved.</span><span>WORKSPACEIRS · {deployEnvironment}</span></div></footer>
 </body></html>;
}