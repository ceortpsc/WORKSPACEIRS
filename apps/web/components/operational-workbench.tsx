...
export default function OperationalWorkbench({routeSlug,triggers,apiEndpoints}:Props){
 ...
 async function run(path:string,body:Record<string,unknown>){
  ...
  try{
   const response=await fetch(path,{method:'POST',headers:{'Content-Type':'application/json','X-Correlation-ID':crypto.randomUUID(),'X-RTPSC-Synthetic':'true','X-Tenant-ID':'synthetic-tenant'},body:JSON.stringify(body)});
 ...
 return <section className="workbench" aria-labelledby={`workbench-${routeSlug}`}>
  <div className="workbench__heading"><div><div className="eyebrow">Operational Wiring</div><h2 id={`workbench-${routeSlug}`}>Run a synthetic control-path check</h2></div><span className="workbench__mode">No taxpayer data</span></div>
  <p>This verifier exercises the deployed workflow and trigger engines using explicitly synthetic identifiers. Production disables this mode and requires a trusted signed identity assertion.</p>
  <div className="workbench__controls">
   ...
   <button className="ross-btn ross-btn--outline" disabled={busy} onClick={()=>run('/api/v1/workflows/transition',{tenantId:'synthetic-tenant',from:'REQUESTED',to:'AUTHENTICATED',taskId:`demo-${routeSlug}`,riskTier:'low',actorRole:'synthetic-verifier'})}>Validate transition</button>
  </div>
